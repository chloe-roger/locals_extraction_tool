...
# This script takes two files in entry : 
#               * the first is a .strings file containing all keys we want to get translations for.
#               * the second is a .xlsm file that has at least 4 pages, the 4th beeing the ALL_STRINGS one, containing all the translations for any key in every language.
# By default, it will create the directories output/de_CH, ouput/en_EU, output/fr_CH, output/it_CH and output/nl_NL. In each of those directories will be created a file called Localizable.strings containing translations for every asked key in the corresponding language.
# You can specify the output directory with the option "-o OUTPUT_DIR"
# You can use the option -u to specify that any translated sentence that contains at least one lower case letter will have its whitespaces and underscores replaced with unsecable character.

# You will need to install gems nokogiri, zip and rubyXL in order to use it.
...
def transform_with_unsecable_char(string)
...
out_strings_path.each do |col, val|
  ...
  File.open(file_name, 'w:UTF-16LE') do |file|
    @keys_to_translate.each do |key|
      ...
      file.puts "\"#{key}\" = \"#{transform_with_unsecable_char(val)}\";"
    end
  end
end
